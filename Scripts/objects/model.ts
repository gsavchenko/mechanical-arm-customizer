...
module objects {
    ...
    export class Model {
        ...
        static ID: number = 0;
        get id(): number { return Model.ID; }
        set id(val: number) { Model.ID = val; }
        
        // stores the current instance's id
        public modelId;
        // stores the object3D (the model itself)
        public model3D: THREE.Object3D;
        // represents the model's position
        public position: THREE.Vector3;
        // stores the name of the model
        public name: string;
        // stores the model that this model is currently attached to
        public attachedTo: objects.Model;
        // represents the first 'attach point' of the model (which is also its position)
        public pointOne: objects.SnapPoint;
        // represents the second 'attach point' of the model which is set programmatically
        public pointTwo: objects.SnapPoint;
        // stores the path of the obj file to load the object from
        public objPath: string;
        // store the path of the mtl file to load the materials from
        public mtlPath: string;
        // represents the 'actual' attachment point (the point that other models will be attached to) of the model
        public _attachPoint: THREE.Vector3;
        // special case one is for models that are attached to model 104 which need to be vertical
        public specialCaseOne:boolean = false;
        // special case two is for when 301 is attached to 105. 301 needs to be flipped
        public specialCaseTwo:boolean = false;
        // special case three is for 5000 series models that are attached to 302. these need to be horizontal instead of 45 degrees
        public specialCaseThree:boolean = false;
        ...
        public expireTimer: number = 3600;
        // represents selected base in 'build-your-system' only one model should be selected at any given time
        ...
        constructor(name: string, modelManager: objects.ModelManager) {
            this.name = name;
        ...
        public updateAttachPoint() {
            this.pointTwo.setPosition(new THREE.Vector3(
                this._attachPoint.x + this.pointTwo.point.position.x,
                this._attachPoint.y + this.pointTwo.point.position.y,
                this._attachPoint.z + this.pointTwo.point.position.z,
            ));
        }

        // used to update the position of the models and attachment points relative to the model it's attached to
        public attachTo(model: objects.Model) {
            ...
            if (!(this.attachedTo === model)) {
                ...
                this.pointOne.setPosition(new THREE.Vector3(
                    this.position.x + this.pointOne.point.position.x,
                    this.position.y + this.pointOne.point.position.y,
                    this.position.z + this.pointOne.point.position.z,
                ));
                ...
                this._attachPoint.set(
                    this.position.x + this._attachPoint.x,
                    this.position.y + this._attachPoint.y,
                    this.position.z + this._attachPoint.z,
                );
        ...
        public getModelObjectCopy(): THREE.Object3D{
