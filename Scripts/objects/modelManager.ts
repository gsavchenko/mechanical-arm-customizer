...
module objects {
    ...
    export class ModelManager {
        ...
        public autoAttach: boolean = true;
        ...
        private _availableModels: objects.Model[] = new Array<objects.Model>();
        // _inUseModels is the list of models currently being used in the scene, not necessarily in the order they should appear
        private _inUseModels: objects.Model[] = new Array<objects.Model>();
        // _mtlLoader a reference to the THREE JS mtl loader
        private _mtlLoader;
        // _objLoader a reference to the THREE JS obj loader
        private _objLoader;
        ...
        private _showSnapPoints: boolean = false;
        ...
        private _armPartsCount: number = 0;
        ...
        public selectBase(name:string): void{
            var tempModel = null;
        ...
        public modifyBaseAfterLoad(model:objects.Model): void{
        ...
        public loadCustomArm(): void{
            var customParts: string[] = JSON.parse(localStorage.getItem("customArm"));
        ...
        public checkCustomArmForWedgeMount(): boolean{
            var customParts: string[] = JSON.parse(localStorage.getItem("customArm"));
        ...
        public addModel(name: string, x:number = 0, y:number = 0, z:number = 0): objects.Model {
            if (this.findByName(name) != null) {
                var m: objects.Model = this.findByName(name);
                ...
                var m: objects.Model = new objects.Model(name, this);
                this.addToLists(m, x, y, z);
                return m;
            }
        }
        ...
        */
        public removePrevious(removeHead: Boolean = false): void {
            if (this.modelOrder.length != 0) {
        ...
        public checkForModel(name: string): boolean {
            var found = false;
        ...
        private loadModel(model: objects.Model): void {
            // Keep a reference to the current object (lost when scope is tightened)
            var thisObject: objects.ModelManager = this;
            this._mtlLoader = new (THREE as any).MTLLoader();
            this._mtlLoader.load(model.mtlPath, function (materials) {
                materials.preload();
                thisObject._objLoader = new (THREE as any).OBJLoader();
        ...
        public removeFromList(array: Array<objects.Model>, model: Model): void {
        ...
        private releaseModel(model: objects.Model): void {
        ...
        private addToLists(model: objects.Model, x:number, y:number, z:number): void {
        ...
        private findByName(modelName: string): objects.Model {
            var foundModel: objects.Model = null;
        ...
        private updateAttachmentPoint(model: objects.Model): void {
            model.pointTwo.setPosition(new THREE.Vector3(
                model._attachPoint.x,
                model._attachPoint.y,
                model._attachPoint.z,
            ));

            model.pointOne.setPosition(new THREE.Vector3(
                model.position.x,
                model.position.y,
                model.position.z,
            ));
